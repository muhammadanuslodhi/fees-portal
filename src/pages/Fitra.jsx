...
export default function Fitra() {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ memberId: members[0]?.id || '', year: filter.year, persons: 1, amountPerPerson: 320, paid: false, notes: '' });
    setShow(true);
  };
  ...
  const save = async (e) => {
    ...
    try {
      if (editing) {
        await api.put(`/fitra/${editing.id}`, form);
        toast.success('Record updated!');
      } else {
        await api.post('/fitra', form);
