...
export default function Zakat() {
  ...
  const [form, setForm] = useState({ memberId: '', year: CURRENT_YEAR, amount: 0, paid: false, notes: '' });
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data } = await api.get('/zakat', { params });
      setRows(data);
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ memberId: members[0]?.id || '', year: filter.year, amount: 0, paid: false, notes: '' });
    setShow(true);
  };

  const openEdit = (row) => {
    setEditing(row);
    setForm({ memberId: row.memberId, year: row.year, amount: row.amount, paid: row.paid, notes: row.notes });
    setShow(true);
  };
  ...
  const save = async (e) => {
    ...
    try {
      if (editing) {
        await api.put(`/zakat/${editing.id}`, form);
        toast.success('Record updated!');
      } else {
        await api.post('/zakat', form);
  ...
  const remove = async (id) => {
    ...
    try { await api.delete(`/zakat/${id}`); toast.success('Deleted'); load(); }
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="page-title">Zakat</h1>
          <p className="page-subtitle">Track annual Zakat payments for all members.</p>
        </div>
              ...
              <tbody>
                {filtered.map((r, i) => (
                  <tr key={r.id} className="table-row group">
                    ...
                    <td className="px-5 py-4 text-surface-600">{r.year}</td>
                    ...
                    <td className="px-5 py-4 text-surface-400 text-xs italic max-w-[150px] truncate">{r.notes || '—'}</td>
                ...
                {!filtered.length && (
                  <tr><td colSpan="8" className="px-5 py-16 text-center text-surface-400 text-sm">No Zakat records found for the selected filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm grid place-items-center z-50 p-4 animate-fade-in">
          <form onSubmit={save} className="card w-full max-w-md animate-scale-in shadow-modal space-y-4">
            <div className="flex items-center justify-between mb-2">
              <div>
                <h2 className="text-lg font-bold text-surface-900">{editing ? 'Edit Zakat Record' : 'Add Zakat Record'}</h2>
                <p className="text-xs text-surface-500 mt-0.5">Annual Zakat contribution</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <select className="input" value={form.memberId} onChange={e => setForm({ ...form, memberId: e.target.value })} required>
                  ...
                  {members.map(m => <option key={m.id} value={m.id}>{m.memberName} — {m.areaId?.areaName || ''}</option>)}
                </select>
              </div>
              <div>
