...
export default function Zakat() {
  ...
  const [form, setForm] = useState({ name: '', year: CURRENT_YEAR, amount: 0, paid: false, notes: '' });
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ name: '', year: filter.year, amount: 0, paid: false, notes: '' });
    setShow(true);
  };

  const openEdit = (row) => {
    setEditing(row);
    setForm({ name: row.name, year: row.year, amount: row.amount, paid: row.paid, notes: row.notes });
    setShow(true);
  };
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">Track annual Zakat payments.</p>
        </div>
          ...
          <div className="hidden sm:block card p-0 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {!filtered.length && (
                  <tr><td colSpan="7" className="px-5 py-16 text-center text-surface-400 text-sm">No Zakat records found for the selected filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
